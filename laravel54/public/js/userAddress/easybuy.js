                    ...
                    if(dataResult[i]["new"] == true){
                        ...
                        if(dataResult[i].townId > 0){
                            addressListHtml += '<div class="ftx06">'+
                                ...
                                '-' +dataResult[i].town+'' +
                                '<a href="javascript:useCurrentRecommend('+dataResult[i].provinceId+', '+dataResult[i].cityId+', '+dataResult[i].countryId+', '+dataResult[i].townId+', \'+dataResult[i].province+\', \'+dataResult[i].city+\', \'+dataResult[i].country+\', \'+dataResult[i].town+\')" clstag="pageclick|keycount|201704194|2" class="ml10 ftx08">使用该地址</a>' +
                                '</div>';
                        }else{
                            addressListHtml += '<div class="ftx06">'+
                                dataResult[i].province +
...
function useCurrentRecommend(provinceId, cityId, countryId, townId, provinceName, cityName, countyName, townName) {
    // 直辖市省和市的id相同
    if(provinceId == cityId){
