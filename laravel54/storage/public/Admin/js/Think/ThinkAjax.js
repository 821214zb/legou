...
var ThinkAjax = {
	...
	ajaxResponse:function(request,target,response){//处理ajax的返回值
      ...
      try{
		...
		*/
		...
		if (this.type == 'EVAL' )
		{
			...
			//将服务器返回的json格式的数据，展示到指定位置
			var options_html="";
			for(var i=0; i<this.data.length;i++){
				options_html+="<option value='"+this.data[i].id+"'>"+this.data[i].title+"</option>";
			}
			$("pid").innerHTML=options_html;
		}
			...
			if (this.showTip && this.info!= undefined && this.info!=''){
				...
				if (this.status==1)
				{
					if ('' != this.image[1])
					{
						...
						$(target).innerHTML	= '<span style="color:blue;">'+this.info+'</span>';//相对定位
					}
				}else{
					if ('' != this.image[2])
					{
	...
	send:function(url,pars,response,target,tips,effect)
	{
		...
		if (this.intval)
		{
			window.clearTimeout(this.intval);//清除定时器
		}
		...
		try {
			...
			var _self = this;
			xmlhttp.onreadystatechange = function (){//监控ajax的通信状态
	...
	{	//获取表单的数据
		...
		// alert(vars);
		//将数据发送到服务器以ajax形式
		this.send(url,vars,response,target,tips,effect);//当前对象的this方法
	},
	// 绑定Ajax到HTML元素和事件
	// event 支持根据浏览器的不同 
